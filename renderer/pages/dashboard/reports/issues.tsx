...
const IssuesRerport = () => {
    ...
    let downloaddata: any;
      ...
      setFetchedSupplier(data.customer)
 ...
 downloaddata = filteredData
  ...
  const downloadPDF = (data: any[], filename = `issues-report-${newT}.pdf`) => {
  const doc = new jsPDF();

  const headers = [
    ["Date", "Reference Number", "Value Date", "Trans Type", "Trans Code", "Customer", "Item", "Part Number", "Location", "Quantity"]
  ];
  ...
  return (
    ...
    <div className='flex justify-between w-full pb-5'>
        <h2 className='text-xl font-bold text-black'>Issues Report</h2>
    </div>   
            ...
            onChange={(e) => setPartnumber(e.target.value)} />
        {
          fetching ? (
            ...
            <Select onValueChange={setCustomer} value={customer}>
                  <SelectTrigger id="customer" className="border-none dark:bg-white dark:text-black">
                  ...
                  {filteredData.length ? (
                    filteredData.map((item:any) => (
                      <TableRow key={item.id}>
                        ...
                        <TableCell className='text-muted'>{item.customer}</TableCell>
