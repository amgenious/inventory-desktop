...
const handleDelete = async (id: any) => {
  try {
    ...
    toast.success("Success! Stock deleted.");
  } catch (error) {
    toast.error(`Failed to delete stock, Error: ${error}`);
  }
};
export const columns: ColumnDef<Stock>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      return (
        <div className="font-medium">
          <p className="text-muted px-1.5">{row.getValue("description")}</p>
        </div>
      );
    },
  },
  ...
  async function onUpdate() {
    ...
    try {
      const response = await fetch(
        `http://localhost:8000/api/v1/stock/update/${item.id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      toast.error(`Failed to update stock, Error ${error}`);
    } finally {
      setIsUpdating(false);
      setOpenSide(false)
    }
          ...
          <form className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-3">
              ...
              {fetching ? (
                ...
                <Select
                  onValueChange={setNewmeasurement}
            ...
            <div className="flex flex-col gap-3">
              ...
              <Input
                ...
                onChange={(e) => setNewmax_stock(e.target.value)}
              />
            </div>
            <div className="flex flex-col gap-3">
              ...
              <Input
                ...
                onChange={(e) => setNewmin_stock(e.target.value)}
              />
            </div>
            <div className="flex flex-col gap-3">
