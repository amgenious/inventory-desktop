...
const handleDelete = async (id: any) => {
  try {
    const response = await fetch(
      `http://localhost:8000/api/v1/customer/${id}`,
      {
        method: "DELETE",
        headers: {
    ...
    toast.error(`Failed to delete customer, Error: ${error}`);
  }
};
...
function TableCellViewer({ item }: { item: any }) {
  ...
  async function onUpdate() {
    ...
    try {
      const response = await fetch(
        `http://localhost:8000/api/v1/customer/update/${item.id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      toast.success("Success! customer has been updated");
      setOpenSide(false)
    } catch (error) {
